...
tick_by_sens <- 0.10        # 10% steps for Sensitivity
tick_by_spec <- 0.01        # 1% steps for Specificity
...
make_panel <- function(dat, xlab, xlim, tick_by = 0.10, tpp = NA_real_) {
  ...
  p <- ggplot(dat, aes(x = est, y = rs_name)) +
    ...
    # put the numeric label slightly to the right of the CI; keep inside extended x-limit
    ...
    scale_x_continuous(
      limits = c(xlim[1], xlim[2] + diff(xlim) * 0.20),     # extend right for labels
      breaks = seq(xlim[1], xlim[2], by = tick_by),
      expand = expansion(mult = c(0, 0))                    # no padding at left edge
    ) +
    labs(x = xlab, y = NULL) +
...
# ===================== Draw figures ==================
p_sens <- make_panel(sens_df, "Sensitivity", xlim_sens, tick_by = tick_by_sens, tpp = tpp_sens)
p_spec <- make_panel(spec_df, "Specificity", xlim_spec, tick_by = tick_by_spec, tpp = tpp_spec) +
  theme(axis.text.y = element_blank())  # hide duplicate RS labels on right

p_combination <- (p_sens | p_spec) +
  plot_layout(widths = c(1, 1)) +
  plot_annotation(
    title = "Diagnostic accuracy of Pluslife sputum swab across reference standards",
    subtitle = "Error bars: 95% Wilson CIs • Labels show n/N",
    theme = theme(plot.title = element_text(face="bold", size=13))
  )

# ---- Save ----
ggsave("D:/Coding/Figures/forest_accuracy2_plss.png",
       p_combination, width = 20, height = 4, dpi = 300)
