...
acc_one_rs <- function(data, index_col, rs_col, rs_name) {
  ...
  n_pos <- tp + fn   # denom for sensitivity
  n_neg <- tn + fp   # denom for specificity
...
make_panel <- function(dat, xlab, xlim, tick_by = 0.10, tpp = NA_real_,
  ...
  ggplot(dat, aes(x = est, y = rs_name)) +
    ...
    scale_x_continuous(
      ...
      expand = expansion(mult = c(0, 0))                 # no left padding
    ) +
    labs(x = xlab, y = NULL) +
...
# ===================== Draw figures ==================
p_sens <- make_panel(sens_df, "Sensitivity", xlim_sens,
                     tick_by = tick_by_sens,
                     grid_col = "grey1",
                     grid_size = grid_size_default,
                     grid_alpha = grid_alpha_default)

# Specificity has dense ticks (every 1%), use even lighter lines
p_spec <- make_panel(spec_df, "Specificity", xlim_spec,
